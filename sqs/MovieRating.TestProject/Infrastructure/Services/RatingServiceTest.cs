...
public class RatingServiceTest
{
    ...
    public async Task GetRatingsTest()
    {
        ...
        var after1Rating = await ratingService.ListAllRatings(expectedMovie.Title);
        
        await ratingService.AddRating(rating2);
        var after2Ratings = await ratingService.ListAllRatings(expectedMovie.Title);
        ...
        await Assert.ThrowsAsync<NoMatchingRatingException>(async () => await ratingService.ListAllRatings("unknown"));
        
        await Assert.ThrowsAsync<ToManyMatchingRatingsException>(async () => await ratingService.ListAllRatings(testMovie.Title));
    }
}
